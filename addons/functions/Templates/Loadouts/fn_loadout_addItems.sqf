...
if (count _uniform > 0) then {
	Debug_1("Trying to get max load of uniform %1", _uniform);
...
if (count _vest > 0) then {
	Debug_1("Trying to get max load of vest %1", _vest);
...
if (count _backpack > 0) then {
	Debug_1("Trying to get max load of backpack %1", _backpack);
